...
//  blog show parcel matter when clicked need to show whole matter like testimonials

// Blog modal logic
document.querySelectorAll(".blog-post-item").forEach(item => {
  item.addEventListener("click", function () {
    const title = item.querySelector(".blog-item-title").innerText;
    const date = item.querySelector("time").innerText;
    const text = item.getAttribute("data-blog-description");

    document.getElementById("blog-modal-title").innerText = title;
    document.getElementById("blog-modal-date").innerText = date;
    document.getElementById("blog-modal-text").innerHTML = `<p>${text}</p>`;
...
document.getElementById("blog-overlay").addEventListener("click", () => {
  document.getElementById("blog-modal").classList.remove("active");
  document.getElementById("blog-overlay").classList.remove("active");
});
